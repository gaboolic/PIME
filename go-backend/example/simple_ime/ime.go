...
// SimpleIME 简单拼音输入法
type SimpleIME struct {
	*pime.TextServiceBase
	pinyin     string              // 当前输入的拼音
	candidates []string            // 候选词列表
	dict       map[string][]string // 简化的拼音词库
}

// NewSimpleIME 创建简单输入法
func NewSimpleIME(client *pime.Client) *SimpleIME {
	return &SimpleIME{
...
// HandleRequest 处理请求
func (ime *SimpleIME) HandleRequest(req *pime.Request) *pime.Response {
...
func (ime *SimpleIME) handleKeyDown(req *pime.Request, resp *pime.Response) *pime.Response {
	keyCode := req.KeyCode
...
func (ime *SimpleIME) lookupCandidates() {
	if ime.pinyin == "" {
		ime.candidates = nil
		return
	}
...
// Init 初始化
func (ime *SimpleIME) Init(req *pime.Request) bool {
	return true
}

// Close 关闭
func (ime *SimpleIME) Close() {
	// 清理资源
}
