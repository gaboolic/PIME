...
func TestFilterKeyDownWhenCompositionEmpty(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "down is ignored", keyCode: 0x27, want: 0},
		{name: "right is ignored", keyCode: 0x28, want: 0},
		{name: "letter is handled", keyCode: 0x4D, want: 1},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resp := ime.filterKeyDown(&pime.Request{KeyCode: tt.keyCode}, pime.NewResponse(1, true))
...
func TestOnKeyDownBackspaceClearsComposition(t *testing.T) {
	...
	if ime.compositionString != "" || ime.showCandidates {
		t.Fatal("expected ime state to reset after backspace")
	}
}
...
func TestOnKeyDownUnhandledKeyReturnsZero(t *testing.T) {
	...
	resp := ime.onKeyDown(&pime.Request{SeqNum: 7, KeyCode: 0x41, CharCode: 'a'}, pime.NewResponse(7, true))
